...
export class Manager extends EventEmitter {
  ...
  // Re-arm an existing pending task with a new trigger, without touching its
  // name or commands. Delay triggers are re-based on "now" so the countdown
  // restarts from the moment of the edit (matching the Compose semantics, where
  // a delay is measured from when it was set).
  async updatePendingTrigger(id, trigger) {
    const task = this.pending.get(id);
    if (!task) throw new Error('Pending task not found');
    task.trigger = this._normalizeTrigger(trigger, Date.now());
    await task.save();
    this.emit('pending:update', task.summary());
    return task;
  }
