...
import { LOG_DIR, RUNNER_DIR, WORK_DIR, SESSION_PREFIX } from './config.js';
...
class Run {
  ...
  _ingest(text) {
    ...
    for (const rawLine of parts) {
      ...
      // Non-marker line: live display + (maybe) per-step output.
      this.manager.emit('run:output', { id: this.id, chunk: line + '\n' });
  ...
  _matchMarker(line) {
    ...
    if ((m = line.match(/\[MUXRUNNER:END:(\d+):rc=(-?\d+)\]/)))
      return { type: 'END', idx: +m[1], rc: +m[2] };
    if ((m = line.match(/\[MUXRUNNER:ABORT:(\d+):rc=(-?\d+)\]/)))
      return { type: 'ABORT', idx: +m[1], rc: +m[2] };
    if ((m = line.match(/\[MUXRUNNER:FINISH:abort=(\d)\]/)))
      return { type: 'FINISH', abort: +m[1] };
    return null;
  }
...
export class Manager extends EventEmitter {
   ...
   */
  async create(name, rawCommands, cwd = WORK_DIR) {
