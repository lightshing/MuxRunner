...
// Statuses shown by /all — the finished or stopped runs.
const FINISHED = ['completed', 'paused', 'closed'];
...
// Inline keyboard shown under bot messages so commands are one tap away
// instead of being typed by hand.
const MENU = {
  inline_keyboard: [
    [
      { text: '📡 活动会话', callback_data: 'sessions' },
      { text: '🗂️ 历史记录', callback_data: 'all' },
    ],
...
export class TelegramBot {
  ...
  async push(text) {
    ...
    return this.sendMessage(this.chatId, text, MENU);
  }

  async sendMessage(chatId, text, replyMarkup = MENU) {
    try {
  ...
  async _dispatch(raw, chatId) {
    const cmd = String(raw).replace(/^\//, '').toLowerCase();
    switch (cmd) {
      case 'sessions':
      case 'status':
      case 's':
  ...
  _renderHelp() {
    return [
      '⚡ <b>MuxRunner bot</b>',
      '',
      '👇 点下面的按钮即可，无需手输指令 · Tap the buttons below — no typing needed.',
      '',
      '📡 <b>活动会话</b> — 正在运行的会话、进度与 attach · live sessions',
      '🗂️ <b>历史记录</b> — 已完成 / 暂停 / 关闭的运行 · finished runs',
  ...
  // History view: only the finished runs (completed / paused / closed), each
  // grouped under its own clearly-distinct status marker.
  _renderAll(limit = 10) {
    const runs = this.manager
      .list()
    ...
    const sections = FINISHED.map((status) => {
      const group = runs.filter((r) => r.status === status);
      if (group.length === 0) return null;
      const items = group.map((r) => {
        const dur = fmtDuration(r.durationMs);
        return `• <b>${esc(r.name)}</b> (${r.done}/${r.total})${r.errored ? ' ❌' : ''}${dur ? ` · ${dur}` : ''}`;
      });
      return [`${statusTag(status)} · ${group.length}`, ...items].join('\n');
    }).filter(Boolean);

    return [`🗂️ <b>历史记录 · Finished runs: ${runs.length}</b>`, '', sections.join('\n\n')].join('\n');
  }
}
