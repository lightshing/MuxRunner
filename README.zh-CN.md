...
- 📡 **实时输出，实时解析**——在抽屉面板里观察运行中的会话：每条命令都是一个可
  ...
  另外还有原始全量流视图。运行期间，原始流会镜像渲染后的终端网格（通过
  `capture-pane` 轮询整段滚动缓冲），因此原地刷新的内容——spinner、跳动的秒数、
  进度条——会显示其当前值，而不再是一片空白。运行结束后，原始流会切换为完整的磁盘
  日志，因此最后一批输出和收尾行始终都在（网格快照在轮询停止的那一刻就冻结了，可能
  漏掉尾部内容）。这类自刷新行在解析后的日志里也会折叠为最后一帧，跳动计数器不会再
  膨胀成「每一帧拼成的一大堆文本」。或者复制 attach 命令，用你自己的终端查看。
- ♻️ **会话会被保留**——无论成功还是出错，运行结束后会话都会保留。**可直接在卡片上
  一键结束会话**（无需先展开抽屉），并通过与界面风格一致的应用内确认弹窗完成（不再
  使用浏览器原生弹窗）。
...
- ⏱️ **挂钟计时**——每条命令的运行时长由 shell 精确测量（bash 微秒级时钟），
  ...
  都会显示。
- ⏲️ **实时计时器**——命令集运行期间，卡片会每秒滚动显示整个任务的已运行时长
  （当前时间 − 开始时间）；实时抽屉中还会同时滚动显示运行总时长，以及**当前这条命令
  已执行的秒数**。
- 🟪 **进度色块**——每张会话卡片为每条命令显示一个小方块（已完成 = 绿色，正在运行的
  那个 = 紫色闪烁，待执行 = 灰色）。每条命令的状态会随列表/快照数据一并下发，因此
  **运行途中刷新页面也能立即正确渲染色块**——不会再出现某条命令长时间运行时（其本身
  不会触发状态更新）整排都是灰色、或缺少闪烁紫色块的情况。
...
2. **运行**——点击 *Run command set*。一个新的 tmux 会话会启动，并打开 *Sessions*
   标签页，显示一张实时卡片，并逐秒滚动显示已运行时长。

3. **观察**——点击卡片（或 *Watch*）打开实时抽屉：流式输出、每一步的状态、attach
   命令、当前正在运行命令的实时秒数计时，以及一个 *Close session* 按钮。你也可以
   **直接在卡片上结束**会话。
...
| **🗂️ 历史记录** / `/all`  | 完整历史，分为两类——✅ **已完成** 与 ⏹️ **已关闭**，按时间倒序，每条附进度、耗时与结束时间（已暂停的运行仍属活动会话，见 📡 活动会话） |
| **❓ 帮助** / `/help`     | 命令列表                                                   |
